...
import { TargetAddresState, TokenAddresState } from "src/atoms/token/multiAccountTokenTransfer";

const SingleTokenMultiTransferList = () => {
    const [tokenAddress, setTokenAddress] = useRecoilState(TokenAddresState);
    const [targetAddres, setTargetAddres] = useRecoilState(TargetAddresState);
                ...
                <Card sx={{ mb: 3 }}>
                    <Box sx={{ px: 2, py: 2 }}>
                        <TextField
                            ...
                            fullWidth
                            value={targetAddres}
                            onChange={(e) => setTargetAddres(e.target.value)}
                        />
                    </Box>
                </Card>
                <Card sx={{ mb: 3 }}>
